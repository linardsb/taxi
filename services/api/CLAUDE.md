...
- A ride is **created** at an entry status (`features/rides/ride-entry.ts`) — creation is the machine's entry, not a transition, because `assertTransition` needs a `from`. Every status change after that goes through `assertTransition` and belongs to #11. Nothing else in the slice may name a ride status.
