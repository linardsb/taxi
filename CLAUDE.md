...
`REDIS_TEST_URL` is **not optional any more — without it `@taxi/api` is RED, not green-and-short.** Two things happen when it is unset, and only the first is the old, documented one: the Redis-backed suites `describe.skip` (33 tests over 4 gated spec files, 2 of which hold nothing else and so report as *skipped suites*), **and** #19 Phase B's `bookings.integration.spec.ts` and `customers.integration.spec.ts` fail outright — 8 tests, because the `POST /rides` idempotency reservation needs Redis and those two specs are not gated. `observed` — `env -u REDIS_TEST_URL COMPOSE_PROJECT_NAME=taxi pnpm --filter @taxi/api test` at #121's rebased head, twice: `8 failed, 33 skipped, 574 passed, 615 total`, `2 failed, 2 skipped, 62 passed, 64 of 66` suites. CI sets it; set it locally to match your `REDIS_PORT`.

**This line has now been wrong three times, and the third time is the instructive one.** It said 24 while the truth was 28; #120's fix pass corrected the digit while inventing a cause ("#19 added gated tests"); and then the sentence itself went stale — "a green gate can be N tests short" describes a gate that no longer exists, so updating 28 → 33 would have left a false sentence around a true number. Re-observe the whole claim, not the digit. The ungated-specs half is filed as **#127**.
