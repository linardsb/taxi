...
**This line has been wrong three times, and the third is the instructive one.** It said 24 while the truth was 33; #120's fix pass corrected the digit while inventing a cause ("#19 added gated tests"); then #121 kept that correct digit and rewrote the *sentence* around it — "RED, not green-and-short" — on a diagnosis the harness rules out (`test/harness.ts:454` overrides `KV_STORE` with `InMemoryKeyValueStore`, so the `POST /rides` idempotency reservation never dials Redis in test). Re-observe the whole claim, not the digit — **and a digit that survives re-observation is not licence to rewrite the sentence around it.** Real integration failures that look like this are far more often the shared-test-DB collision documented in the next paragraph.
