...
Redis-backed suites are **opt-in**: without `REDIS_TEST_URL` they `describe.skip`, so a green gate can be 28 tests short — spread over 4 gated spec files, 2 of which hold nothing else and so report as *skipped suites* (`observed` — `@taxi/api` without it on #120's branch: `28 skipped, 492 passed, 520 total`, 2 skipped suites of 57). CI sets it; set it locally to match your `REDIS_PORT`. **This is a moving number and this line has now been wrong twice** — it said 24 while the truth was 28, and #120's fix pass then corrected the digit while inventing a cause ("#19 added gated tests"; #19 touches no gated file, and `git log` puts the last change to `test/driver-location-store.contract.ts` in #18). Re-observe it rather than quoting it, and if you correct it, check *what moved it* rather than assuming your own branch did.
