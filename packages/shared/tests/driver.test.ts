...
import { vehicleCreateSchema, vehicleUpdateSchema } from "../src/schemas/vehicle";
...
describe("driverProfileUpdateSchema", () => {
  ...
  it("rejects an empty patch and an empty language list (failure)", () => {
    ...
    expect(driverProfileUpdateSchema.safeParse({ spokenLanguages: [] }).success).toBe(false);
  });

  it("parses a language change on its own (expected)", () => {
    const parsed = driverProfileUpdateSchema.parse({ spokenLanguages: ["lv", "ru"] });
...
describe("driverStatusUpdateSchema", () => {
  it("accepts the two presences a driver owns (expected)", () => {
    expect(driverStatusUpdateSchema.parse({ status: "online" }).status).toBe("online");
    expect(driverStatusUpdateSchema.parse({ status: "offline" }).status).toBe("offline");
  });
  ...
  it("rejects on_ride (failure — #11 owns that transition)", () => {
    ...
    expect(driverStatusUpdateSchema.safeParse({ status: "on_ride" }).success).toBe(false);
  });
});
...
describe("vehicleCreateSchema", () => {
  ...
  it("drops a client-supplied driverId (edge — ownership comes from the JWT)", () => {
    ...
    // else's account.
    const parsed = vehicleCreateSchema.parse({ ...body, id: uuid, driverId: uuid });
    expect(parsed).not.toHaveProperty("driverId");
    expect(parsed).not.toHaveProperty("id");
  });

  it("rejects a year before the schema's floor (failure)", () => {
    expect(vehicleCreateSchema.safeParse({ ...body, year: 1980 }).success).toBe(false);
  });
});
