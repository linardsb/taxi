...
describe('otpRequestSchema', () => {
  ...
  it('rejects a national-format phone number (failure)', () => {
    expect(
      otpRequestSchema.safeParse({ phone: '26123456', role: 'rider' }).success,
    ).toBe(false);
  });

  it("rejects role 'admin' — privilege escalation blocked at the contract (failure)", () => {
    expect(otpRequestSchema.safeParse({ phone, role: 'admin' }).success).toBe(
      false,
    );
    expect(
      otpRequestSchema.safeParse({ phone, role: 'dispatcher' }).success,
    ).toBe(false);
  });
});
...
describe('otpVerifySchema', () => {
  it('parses a 6-digit code (expected)', () => {
    expect(otpVerifySchema.parse({ phone, code: '012345' }).code).toBe(
      '012345',
    );
  });

  it('rejects a 5-digit code and a non-numeric one (failure)', () => {
    expect(otpVerifySchema.safeParse({ phone, code: '12345' }).success).toBe(
      false,
    );
    expect(otpVerifySchema.safeParse({ phone, code: '12345a' }).success).toBe(
      false,
    );
  });
});
...
describe('authSessionSchema', () => {
  ...
  it('rejects a Date where the wire demands an ISO string (edge — wire vs domain)', () => {
    const withDate = {
      ...session,
      user: { ...session.user, createdAt: new Date(at) },
    };
