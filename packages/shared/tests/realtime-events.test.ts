...
describe("driverQueueEventSchema", () => {
  const base = { driverId: uuid, geozoneId: otherUuid, geozoneSlug: "rix", position: 1, size: 4, at };
  ...
  it("rejects a position of 0 — positions are 1-based (failure)", () => {
    expect(driverQueueEventSchema.safeParse({ ...base, position: 0 }).success).toBe(false);
  });
});
...
describe("rideOfferRevokedEventSchema", () => {
  ...
  it("carries the other two revoke reasons (edge)", () => {
    expect(rideOfferRevokedEventSchema.parse({ ...base, reason: "taken" }).reason).toBe("taken");
    expect(rideOfferRevokedEventSchema.parse({ ...base, reason: "cancelled" }).reason).toBe(
      "cancelled",
    );
  });

  it("rejects an unknown revoke reason (failure)", () => {
    expect(rideOfferRevokedEventSchema.safeParse({ ...base, reason: "declined" }).success).toBe(
      false,
    );
  });
});
...
describe("rideAssignedEventSchema", () => {
  ...
  it("parses an auto-matched assignment with no dispatcher (expected)", () => {
    const parsed = rideAssignedEventSchema.parse({ ...base, source: "auto_match" });
  ...
  it("rejects a dispatcher assignment without dispatcherId (failure — the audit rule)", () => {
    ...
    expect(rideAssignedEventSchema.safeParse({ ...base, source: "dispatcher" }).success).toBe(false);
    expect(
      rideAssignedEventSchema.safeParse({ ...base, source: "dispatcher", dispatcherId: null })
        .success,
    ).toBe(false);
  });
});
...
describe("dispatchBoardEventSchema", () => {
  const base = {
    ...
    at,
    rides: [
      { rideId: uuid, status: "requested", pickup, driverId: null, unclaimedSeconds: 42 },
    ],
  ...
  it("parses an empty board before any orders land (edge)", () => {
    const parsed = dispatchBoardEventSchema.parse({ ...base, rides: [], drivers: [] });
...
describe("dispatchUnclaimedEventSchema", () => {
  const base = { rideId: uuid, pickup, requestedAt: at, unclaimedSeconds: 90, offerAttempts: 3 };
  ...
  it("allows zero offer attempts — nobody was even in range (edge)", () => {
    expect(dispatchUnclaimedEventSchema.parse({ ...base, offerAttempts: 0 }).offerAttempts).toBe(0);
  });

  it("rejects a negative unclaimed duration (failure)", () => {
    expect(dispatchUnclaimedEventSchema.safeParse({ ...base, unclaimedSeconds: -1 }).success).toBe(
      false,
    );
  });
});
...
describe("rideOfferEventSchema — the wire projection", () => {
  ...
  it("survives the JSON round-trip a socket actually performs (edge)", () => {
    ...
    expect(domain.expiresAt.getTime()).toBe(Date.parse("2026-08-03T10:00:20.000Z"));
  });

  it("rejects a Date where the wire promises a string (failure)", () => {
    expect(
      rideOfferEventSchema.safeParse({ ...wire, expiresAt: new Date(wire.expiresAt) }).success,
    ).toBe(false);
  });
});
...
describe("event typing", () => {
  it("declares every outbound timestamp as a string, never a Date (type-level)", () => {
    ...
    type OfferPayload = Parameters<ServerToClientEvents[typeof RT.rideOffer]>[0];
    const _expiresAt: string = {} as OfferPayload["expiresAt"];
    const _sentAt: string = {} as OfferPayload["sentAt"];
    type StatusPayload = Parameters<ServerToClientEvents[typeof RT.rideStatus]>[0];
  ...
  it("keeps the untrusted inbound payload unreadable without a parse (type-level)", () => {
    ...
    const handler: ClientToServerEvents[typeof RT.driverLocation] = (payload) => {
