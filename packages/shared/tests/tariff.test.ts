...
describe('rideTariffSchema', () => {
  ...
  it('accepts a zero minimum fare — no floor is a real configuration (edge)', () => {
    expect(
      rideTariffSchema.parse({ ...base, minimumFareCents: 0 }).minimumFareCents,
    ).toBe(0);
  });

  it('rejects a fractional per-km rate — integer cents only (failure)', () => {
    expect(
      rideTariffSchema.safeParse({ ...base, perKmCents: 80.5 }).success,
    ).toBe(false);
  });
  ...
  it('requires baseCents — config, not constant (failure)', () => {
    ...
    if (!result.success) {
      expect(
        result.error.issues.some((issue) => issue.path.includes('baseCents')),
      ).toBe(true);
    }
  });
  ...
  it('rejects a category outside RIDE_CATEGORIES (failure)', () => {
    expect(
      rideTariffSchema.safeParse({ ...base, category: 'helicopter' }).success,
    ).toBe(false);
  });
});
