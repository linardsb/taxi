...
describe("platformConfigSchema", () => {
  ...
  it("accepts a 0% commission — the S6-7 pilot scenario (edge)", () => {
    expect(platformConfigSchema.parse({ ...base, commissionPct: 0 }).commissionPct).toBe(0);
  });
  ...
  it("requires commissionPct — config, not constant (failure)", () => {
    ...
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.path.includes("commissionPct"))).toBe(true);
    }
  });
});
