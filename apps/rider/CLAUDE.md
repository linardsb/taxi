...
- Multi-stop, scheduled rides, multi-taxi and rider bids are **not sent** by this app — **by choice, not by schema**. `rideQuoteBodySchema` picks `stops`, and `rideRequestBodySchema` carries `stops`, `scheduledFor`, `vehicleCount` and `offeredPriceCents`; the shared contract would accept every one of them. What keeps them out is `use-quote.ts` sending `{ pickup, destination }` and `use-book-ride.ts` sending pickup, destination and payment method. There is no guard here to trust — adding a field to either call ships it.
