...
/** `const t = useT(); t('driver.home.go_online')` — the catalog, bound to the device language. */
...
/** For code that runs outside React (the foreground-service notification text). */
export function tNow(): T {
  const lang = deviceLanguage();
  return (key, params) => formatMessage(lang, key, params);
}
