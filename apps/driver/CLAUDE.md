...
- Status must mirror `DriverStatus` from `@taxi/shared`. The app owns INTENT (SecureStore `sakta.driver.intent`); the server owns FACT. On every reconnect and cold launch with intent `online` the app re-asserts `PUT /drivers/me/status online`; a 409 (`vehicle_required`) wins and flips the toggle with the reason. A mid-ride re-assert is not a 409: the api acks it with the `on_ride` profile (only `offline` 409s while held), which the app reads as the server holding us — the stream stays up.
