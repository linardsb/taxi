...
| Live CI runs (new advisory / changed lockfile with no new id / unchanged lockfile) | new advisory, `lodash@4.17.20` at the root: run 34514881914, `audit-diff` **failure** in 10 s (`66 at base, 71 at HEAD`, the five ids above), `ready` failure, PR left a draft · changed lockfile with no new id, `zod@3.25.76` declared at the root (already in the lockfile for `packages/shared`): run 34515354578, success in 10 s (`66 at base, 66 at HEAD, none new`), PR flipped ready · unchanged lockfile: run 34515835893, success in 7 s on the short-circuit line; PR A's own first run 34471269249 took the same path, 7 s | `observed` 2026-09-10 on throwaway PR #179 (`probe/audit-diff-165` off `e75c677`, three pushes, closed after; #165 T7) and on PR #167's first run. Durations are the job's `completed_at − started_at`. The table with heads and job URLs is on PR #167's body, under *T7*. Not "revert" as the plan wrote: a plain revert restores the lockfile and takes the short-circuit, so the middle case needs a lockfile change that adds no package |
