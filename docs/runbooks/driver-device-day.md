...
**Why a still pin on `/dispatch` proves nothing on its own.** The phone streams a fix every 4 s whether or not
it moves — the throttle is time-based (`fix-throttle.ts:9`, `MIN_FIX_INTERVAL_MS = 4_000`) and `distanceInterval`
is `0` (`location-options.ts:20`), both by design. So a stationary phone's pin does not move, and **position alone
cannot tell a live stream from a dead one**. That is why step 5 reads the api console and not the map.

What has changed since this sheet was first written: the board now renders the freshness as well as the position.
[#234](https://github.com/linardsb/taxi/issues/234) gave `apps/dispatch/src/features/board/` a production reader
for the `lastSeenAt` that `board-state.ts` had been folding in with nobody consuming it — a «Šoferi» panel, present
in both the zones and map views, whose per-driver row reads «Raida» or «Klusē MM:SS» against the shared
`DRIVER_LOCATION_TTL_SECONDS`. So the board is now a usable second surface for step 5, and step 6's tracking page
is no longer the only one. The api console remains primary for the reason above: it is the only surface that shows
each accepted fix *arriving*, which is what the 4 s claim is about.
