...
in both the zones and map views, whose per-driver row reads «Raida», «Klusē MM:SS» or «Nav signāla» against the
shared `DRIVER_LOCATION_TTL_SECONDS`. So the board is now a usable second surface for step 5, and step 6's tracking
page is no longer the only one. Read «Nav signāla» as *the console cannot vouch for this driver* rather than as a
verdict on the phone: it covers a driver with no recorded position at all (going online never records one — only an
accepted fix does) **and** the case where the board itself has lost its frames, which it cannot distinguish from a
driver going quiet. Both are reasons to trust the api console over the board, which is why the board corroborates
and does not decide. The api console remains primary for the reason above: it is the only surface that shows
...
| The offers / active-ride device pass | #15 — `.claude/plans/driver-offers-active-ride.md:506` §Level 4 |
