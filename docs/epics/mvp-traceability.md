...
| Deploy (Hetzner VPS) | #13 | built (2026-08-25); box provisioning + first deploy are runbook steps | **host changed Railway → Hetzner CX22** (2026-08-16, architecture amendment). Capability unchanged; ops responsibilities Railway used to own (backups, TLS, deploys) move to us — see the amendment's mitigation table. The maps boot gate is behind `ALLOW_STUB_MAPS_PROVIDER` for this ticket; `OsrmMapsProvider` is #134 and deletes the switch. Cash-only posture: card charges are **refused** in production (`CardPaymentsDisabledProvider`), never stubbed |
