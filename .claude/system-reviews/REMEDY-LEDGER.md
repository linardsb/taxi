...
**Amended 2026-09-21 (#136 / PR #245), the later of the two #136/#137 amendments below.** **L20 applied and closed — and, like L18 before it, it RECURRED in the loop that applied it**: PR #245's F1 probe used a same-shaped substitute for the review's ReDoS input and went **green on the vulnerable body in 8 ms**. The implementer caught it; the skill did not. L26 added as closed (new, applied same loop). L23, L24 and L25 appended as open. **Open is now 5: L17, L21, L23, L24, L25.** L25 is ranked first for the next apply slot — it is the #87 mechanism one layer earlier than #87 found it.

**Amended 2026-09-21 (#137 / PR #241).** L18 applied and closed — *after recurring in the loop that applied it*, which is the ledger's own thesis observed rather than argued. L22 added as closed (new, applied same loop). L20 and L21 appended as open. **Open is now 3: L17, L20, L21.** L20 is ranked first for the next apply slot.
...
| L21 | The `dist` byte-compare as a reusable "did this change behaviour?" instrument: `services/api/tsconfig.build.json` excludes specs, so building before and after a comment/type-only pass and `cmp`-ing every emitted `.js` survives spec additions, comment edits and type-only changes — all of which break a `git diff` filter. Candidate for `.claude/references/` | this loop, 2026-09-21 (PR #241 round 1 — 158 emitted `.js`, 0 differing, which is what kept five container probes `observed` with no image rebuild) | C4 | — | not a pattern yet: **one use**. Logged rather than promoted; promote on a second independent use |
| L23 | `piv-review-pr`'s severity table conflates "blocks the merge" with "is dangerous". PR #245's only High is `codeql` red on `js/polynomial-redos`; the review's own prose says it is **not exploitable in this tree** (both call sites pass an operator-set, `z.string().url()`-validated env value). It is High because `codeql-gate.sh` fails on an open high-severity alert the base does not carry and `ready` needs it. The table is the surface people read. Costs one column, or one kind-word beside the severity | this loop, 2026-09-21 (#136 / PR #245 F1) | C7 | — | absent — `grep -in "blocks the merge" .claude/skills/piv-review-pr/SKILL.md` → 0 hits |
| L24 | The claims-outlived-their-subject sweep runs in `piv-fix-review-findings` (§2's value-sweep), one step after the review that should have caught them. On #136 it found **five** stale figures the review had missed, and four of the review's ten findings (F3, F4, F5, F10) were the same class. Move a value-grep pass into `piv-review-pr` rather than leaving it to the fixer | this loop, 2026-09-21 (#136 / PR #245) | C1 | — | not present as a review step — `grep -in "value sweep\|value-sweep" .claude/skills/piv-review-pr/SKILL.md` → 0 hits |
| L25 | `piv-plan-implementation`'s Fact Verification block (L1/L13) checks a plan's claims **about the codebase** against real artifacts. It does not check a plan's **rationale or security claims** against the code that would have to be true for them. #136's plan `:492` instructed the docblock to say `TRACKING_VIEW_MAX_PER_WINDOW` "rate-limits guessing"; it does not (it is keyed per token, and `notifications.policy.ts` — edited in the same diff — says so), and that false justification is what licensed cutting the token from 128 bits to 96 | this loop, 2026-09-21 (#136 / PR #245 F3) | C1 | — | absent — the block's scope words are codebase artifacts: `grep -in "read out of the source, decorators included" .claude/skills/piv-plan-implementation/SKILL.md` → 1 hit, and nothing beside it covers a rationale claim. **Ranked first for the next apply slot** — this is the #87 mechanism one layer earlier than #87 found it |

## Closed since the 2026-09-04 loop

| ID | Landed in | Verify at HEAD |
|----|-----------|----------------|
| L20 | **2026-09-21** (#136 / PR #245) — **and it RECURRED before it was applied**, which is now the second row to do so after L18. `piv-fix-review-findings` §2 step 3 now requires the probe to use the input or mutation the finding **names**, verbatim, before a cleaner one is written, and to say so in the fixes report when the finding's own input cannot be reproduced. Both recurrences are cited in the step: PR #245 F1 (`'https://a' + '/'.repeat(100_000)` **passed on the vulnerable body in 8 ms**; with the review's trailing `x`, 8892.7 ms against a 250 ms bound) and PR #241 L1 (`14 passed, 14 total` through the reviewer's exact edit) | `grep -in "mutation the finding names" .claude/skills/piv-fix-review-findings/SKILL.md` → **1 hit, line 91** — the row's own absence locator, now reproducing |
| L26 | **2026-09-21** (#136 / PR #245), new this loop and applied in it — `piv-plan-implementation`'s ACCEPTANCE CRITERIA block now requires naming the **property** and the cheapest oracle for it before an AC is written off as unperformable here. The prior rule at the same site asks only whether *this machine* can perform the verification, which is a question about the instrument. #136's AC #0 said "verified handset"; the property was "does a bare `host.tld/path` linkify", and an Android emulator plus `NSDataDetector` answered it in about an hour, both available the whole time | `grep -in "find the cheapest oracle for IT, not for the instrument" .claude/skills/piv-plan-implementation/SKILL.md` → **1 hit, line 452** |
| L18 | **This loop, 2026-09-21** (#137 / PR #241) — and it RECURRED before it was applied: with `arguments: [plan]` still declared, `system-execution-report` bound `$plan` to the literal string `PR` from the sentence "PR #241 — the SMS provider switch slice…" and rendered **"Plan file: PR"**. Logged 2026-09-04 against that exact file, unapplied for the loops between. Both remaining skills now read `$ARGUMENTS` as prose (`opportunity-scan:27`'s pattern) | `grep -n "^arguments:" .claude/skills/*/SKILL.md` → **1 hit**, `system-evolution-review:5`, which carries its own guard text and caught this loop's mis-invocation. `grep -c 'as \*\*prose\*\*' .claude/skills/system-execution-report/SKILL.md .claude/skills/piv-fix-review-findings/SKILL.md` → 1 each |
| L22 | **This loop, 2026-09-21** (#137 / PR #241) — VALIDATE steps that assert test A catches a mutation AND test B does not must run BOTH. #137's plan taught the false half at four sites, its VALIDATE step passed truthfully, and the claim was copied into a new source comment before PR #241's M1 caught it | `grep -n "runs BOTH under that" .claude/skills/piv-plan-implementation/SKILL.md` → 1 hit |
