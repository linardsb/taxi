...
Every one of the five divergences is justified and documented, and three of them (D1, D2, D4) show the implementer reasoning about the *codebase* over the plan text — the good shape. Three points off, and unlike Phase A they are not all the same point:
...
- [x] Add a **guarantees pass** beside the existing numbers pass in Phase 4, scoped to PRs whose base moved. Suggested text:
...
- **Guarantees do not survive a moving base, and nothing checks them.** D2 was predicted in a code comment months before it fired, and the comment was still standing as a warning after the event it warned about.
