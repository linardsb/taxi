...
- **`attempts` is consistent with the engine** — `offers.length` matches what `countAttempts` counts, which is the number `offerNext` compares against `MAX_OFFER_ATTEMPTS`. The board's count and the engine's give-up threshold are the same number, which is the right choice for a board whose job is explaining the engine. **Superseded by round 3 (F2/F3) — this bullet recommends what `main` now forbids.** #120's H3 made the engine's budget release-scoped while the board's stayed cumulative since booking, so the two counts agree only while the ride has never been released; the cap comparison was removed in `794d602`. See `cascade.ts:170-188` on `main`, which closes with *"Do not reinstate the guard without a release-scoped count in the frame."*
