...
Also corrected: **`:474`** cited `driver-location.policy.ts:30,33` for the 75 s derivation; the constants are at **`:40,46`** (`observed` — `grep -n` on the file). The arithmetic and the figure are right; the provenance ref was not.
