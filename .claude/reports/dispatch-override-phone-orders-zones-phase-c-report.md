...
**One claim in the pre-rebase report and PR body was false, not merely stale:**
...
**A second claim was written here and was itself false — corrected in review round 3.** This report and the `CLAUDE.md` paragraph both said that without `REDIS_TEST_URL` the api suite is RED rather than green-and-short, because Phase B's `bookings.integration.spec.ts` and `customers.integration.spec.ts` need Redis and are not gated. **It does not reproduce and the mechanism is ruled out by the harness**: `test/harness.ts:454` overrides `KV_STORE` with `InMemoryKeyValueStore`, both specs build through `createTestApp`, and `rides.service.ts:120` reaches `setIfAbsent` only through that injected token — so the idempotency reservation cannot dial Redis in test. `observed` at `feed712`, `env -u REDIS_TEST_URL COMPOSE_PROJECT_NAME=taxi pnpm --filter @taxi/api test`: **exit 0**, `Tests: 33 skipped, 582 passed, 615 total`; the two specs run alone are **2 suites, 13 tests, exit 0**. The M5 digit correction (24 → 33) was right; the sentence rewritten around it was not. The `8 failed` the author saw was real output attributed to a cause the code excludes — the leading candidate is the shared-test-DB collision `CLAUDE.md` documents one paragraph later. **#127** is re-scoped to finding that cause rather than gating the specs.
...
| H2 | «Nākamais» named offline drivers, who are in the queue snapshot ON PURPOSE but are never in the engine's candidate set (`findNearest` is the online set; `toCandidates` requires `status === 'online'`) | `nextInQueue` requires `status === 'online'`; docblock now says plainly this is a heuristic over the rank, not a replay of `findCandidates`. **The cap half of this fix was retired in review round 3 (F2)** — the rebase made the engine's budget release-scoped, so a cap comparison here misreports a released ride's cascade as spent; see the revised docblock | `cascade.spec.ts` "skips an offline driver when naming who is next" and "still names who is next at the attempt cap, because the board cannot see the engine budget" |
...
| L9 | Two fixtures built rows no writer emits — the round-1 pattern restated inside the commit answering it. Lidosta's two-entry rank held positions 4 and 5 (`snapshotFrom` derives `index + 1`, so a two-entry rank is 1 and 2); the attempts case wrote four offer rows for one driver (`findTriedDriverIds` + `offerNext` mean five rows are five distinct drivers) | Positions 1/2; five distinct tried drivers **plus a sixth online untried one** | none — fixture producibility, not behaviour, and the mutation run does not discriminate it: revert these fixtures alone and both cases stay green. They go red pre-fix because of H1 and H2, not because of the row shape. The sixth untried driver is not new coverage either — it **preserves** discrimination the producibility fix would otherwise have destroyed: with only the default two-entry rank, five distinct tried drivers exhaust it, `nextInQueue` returns null via the `find`, and the case would pass with or without the `MAX_OFFER_ATTEMPTS` guard |
