...
- **i18n conflict** → 80 lines of catalog strings into `packages/shared/src/i18n/{lv,ru,en}.ts` (UPDATE), `isMessageKey` kept in `i18n.ts` (UPDATE)
...
- **Issues filed** → **#126** (balance eligibility), **#127** (ungated Redis specs)
...
4. **A third issue was filed beyond the two the handoff named** — #127. Phase B's `bookings`/`customers` integration specs need Redis and are not gated, so without `REDIS_TEST_URL` the api suite is red rather than short. Found while re-deriving `CLAUDE.md:43`'s skip figure.
