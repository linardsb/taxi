...
| F5 | Med | «Every row below says which [APK]» was false for 13 of 14 rows; the absolute *"No #15 step result is taken from this APK"* is contradicted by steps 1, 2, 3a; `16:23:16` was the one timestamp with no timezone | Run-environment row now **assigns every step that ran to a pass, except 10** (see below); `:188` narrowed to *"No step requiring a ride payload"*; `16:23:16` → **`15:23:16Z`** (this pass first wrote it as UTC; round 2's F11 corrected it — the emulator's clock follows the host's BST) | Settled wrongly here, corrected in round 2 — see §"F5's timezone" |
