...
> - *313–361 s* is **run wall**, not the `check` job, and all 8 runs predate the repo going public. The eight ids named on the line above are real and give exactly that range, as do the plan's slightly earlier eight — both windows share their min (34373346365, 313 s) and max (34375354100, 361 s); re-derived under **2026-09-10 (#175 M2)** in `.claude/plans/ci-no-model-pr-gate.md`, which is the one citation for the run set. `check` re-derived on PR #167's own four runs (34471269249, 34471798333, 34472223208, 34476424460) is **195–206 s**; push-to-flip is about 3.5 min, not six (F10). Run wall is unusable for 34472223208 (2693 s — it sat between the PAT retries), which is the second reason to cite the job.
...
5. **T7 not run in this session.** The task's own text places it after PR A's `ci.yml` is on `main`. PR A's own run supplies the unchanged-lockfile URL; the red and revert URLs are owed after merge. `expected`, not verified: a throwaway opened against PR A's branch would run PR A's workflow and could supply them before merge; the plan's parenthetical says otherwise and I did not test it.
   *Correction 2026-09-10, after merge:* T7 ran on throwaway PR #179 — red run 34514881914, green-with-changed-lockfile run 34515354578, short-circuit run 34515835893. The plan's AMENDMENTS entry of that date is the single citation; the table with heads and job URLs is on PR #167's body. The "before merge against PR A's branch" idea above was never tested and is moot.
