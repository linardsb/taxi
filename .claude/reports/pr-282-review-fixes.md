...
| H2 | `search-sheet.tsx:103` ticks every 1 s. `:281-285` puts `cooldown` in the Banner `text`, and Banner's effect is keyed on `[text]`. | T0 P6: the countdown stays visible. The Banner is silent while its error came from a 429, and each 429 (`:182`, `:231`) announces once with `retryAfterSeconds`. Test: 3 s of fake timers gives one call containing `5`; past the countdown, still one; then an offline error is announced once. **Superseded** by round 2 M1: the offline case cannot reach the cooldown window; see `pr-282-review-fixes-round2.md`. |
...
- **H2.** A silence flag left over from a 429 could mute the next, different error. Test (T0 P6): an offline failure after a 429 is announced once. **Superseded** by round 2 M1: that test could not reach the cooldown window (the field does not search while cooling down), so it pinned nothing; the replacement is in `pr-282-review-fixes-round2.md`.
