...
- **P7, the replacement command, against real Postgres — the shipped form, not a proxy.** `docker exec -i taxi-db-1 pg_dump -U taxi -d taxi -Fc > t.dump` (57 015 bytes), then `docker compose … exec -T db pg_restore --list < t.dump` → **exit 0, 147 lines**, including `291; 1259 21319 TABLE public geozones taxi` (`docker exec -i` gives the identical result). Two assumptions, both confirmed rather than reasoned: a custom-format archive lists from a non-seekable stdin, and compose's `exec -T` forwards that stdin through instead of eating it — which is the same class of thing N1 was.
