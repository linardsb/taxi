...
| A13 | `packages/shared/src/i18n.ts` (UPDATE) — 33 `console.*` keys across LV / RU / EN (`observed`: distinct top-level keys in the `lv` catalog, 46 on main → 79 on this head; every added key is a `console.*` one. Written as 30 first and inherited into the PR body; the commit as reviewed added 28, and the #120 review-fix pass added 5 more. An intermediate correction gave the provenance as "99 added lines ÷ 3 catalogs" — that was back-derived from 33 × 3 and is wrong: `git diff --numstat` reports **131** added lines, because 10 are comments and 22 are wrapped values) |
...
**On "24 short":** root `CLAUDE.md` said 24 and quoting it here was wrong — the real figure is 28, which the intermediate `478 passed, 28 skipped` run below already showed and this line inherited past. Re-observed after the #120 review-fix pass: `env -u REDIS_TEST_URL` on this head gives `28 skipped, 492 passed, 520 total`, 2 skipped suites of 57. `CLAUDE.md:43` is corrected to 28.

**And the first correction got the CAUSE wrong**, which is the same defect one level up: it said this branch's own gated tests moved 24 → 28. **They did not.** `git diff origin/main...HEAD` over all four gated spec files and both store contracts is **empty** — #19 touches none of them, so main skips 28 as well. `git log` puts the last change to `test/driver-location-store.contract.ts` in `2859446` (#18), already on main. 24 was stale before this branch existed. Caught by the round-2 review; the wrong attribution originated in the round-1 review and was inherited rather than audited — exactly what root `CLAUDE.md` warns about, committed while fixing an instance of it.
