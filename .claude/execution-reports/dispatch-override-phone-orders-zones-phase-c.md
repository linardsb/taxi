...
- **The i18n split absorbed Phase C cleanly.** **16 new keys per language — net +15**, since `console.zone_none` was reworded to `console.zone_none_configured` — taking each dictionary **136 → 151** keys, byte-identical key sets across `lv`/`ru`/`en`. `observed` — `git show <rev>:packages/shared/src/i18n/lv.ts | grep -cE "^\s+'[a-z_.]+':"` at `d444c72` and `59b3feb`, same for `en`/`ru`. **Neither "80 lines" nor "59 lines" counted strings**: 59 is the three-file *added-line* total (`lv +25`, `en +17`, `ru +17`), whose surplus over the keys is 8 LV-only comment lines and one wrapped value; 80 adds `i18n.ts`'s 21 assembly and type lines. The line counts are real and name nothing anyone asked about — a right figure under a wrong noun, which is the #87 shape. The cap problem in `i18n.ts` was solved by **#122's split, not by Phase C**: `0e2d0d7` took the file 498 → 26 (`observed`, same command over `0e2d0d7^..0e2d0d7`), and over Phase C's own stated diff base `d444c72..59b3feb` the file **grew** 26 → 47. The 454 → 47 drop quoted in earlier surfaces straddles the rebase — 454 is `d67c82a`, the pre-rebase Phase C tip — and credits Phase C with #122's work. See D3.
- **Review round 3 caught two Highs created by the rebase, not by Phase C's own work** — an argument for reviewing the rebase commit as its own event, and it worked. **But "rounds 1 and 2 could not have" is false, and the true version is worse.** `pr-121-review.md:155` — round 1 — named the exact coupling in advance: *"fixing H3 redefines `countAttempts`, the same row set `cascade.ts` projects as `attempts` … After the #120 rebase this needs `cascade.spec.ts` and `board.service.spec.ts` re-run, not a fresh review."* The tripwire was written **and** nominally acted on; it was discharged in its weaker form — re-run the specs, which passed, because the divergence is structurally untestable — instead of by re-deriving the relationship. That strengthens ACTION 1 in the system review rather than softening it: a green run closed a note that only a re-derivation could close.
...
- **Type**: Plan assumption wrong — invalidated by a sibling PR merging first, in a file whose own comment had named the exact invalidating condition in advance: *"#120's H3 redefines what `countAttempts` counts; if it stops meaning 'rows for this ride', this comparison needs re-deriving with it"* (`cascade.ts:180-181` **at `feed712`**, the pre-fix revision). `794d602` rewrote that docblock, so the same line numbers on `main` now hold the replacement text, not this comment.
