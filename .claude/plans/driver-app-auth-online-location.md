...
14. **Toggle OFF mid-ride (issue #141 — SUPERSEDED, do not run this step)**: #141 landed the fix — run `docs/runbooks/driver-device-day.md` instead, which owns the corrected sheet and states its own pass conditions. *Historical, for the record only*: as written here the step force-assigned the driver from `/dispatch`, then tapped the toggle OFF, and expected to FAIL — the app tore the stream down before the api refused with `driver_on_ride`, so the pin froze on the board while the row still said `on_ride`, and one tap back ON recovered.
