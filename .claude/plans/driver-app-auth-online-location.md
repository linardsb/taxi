...
14. **Toggle OFF mid-ride (issue #141, expected to FAIL)**: force-assign this driver from `/dispatch`, then tap the toggle OFF. Known defect: the app tears the stream down before the api refuses with `driver_on_ride`, so the pin freezes on the board while the row still says `on_ride`. Recorded, not fixed in this PR. One tap back ON recovers. **Superseded: #141 landed the fix — run its run sheet (`.claude/plans/driver-toggle-off-mid-ride-held.md` §Level 4 §1) instead; the toggle now springs back to ON with a banner and the pin keeps moving.**
