...
12. **TalkBack banner (owed to review F28/F47)**: enable TalkBack, force a banner (deny foreground location, tap the toggle) → the text is spoken ONCE — the Android live region; the explicit announce is iOS-only. A double read is the F28 regression, and **silence is the F47 one**: the Android announce was removed on the expectation that `accessibilityLiveRegion="polite"` fires for a freshly mounted view, which no device has confirmed. This step decides it.
13. **Deny background location, then go online (owed to review F36)**: `adb shell pm revoke lv.saktacab.driver android.permission.ACCESS_BACKGROUND_LOCATION`, tap the toggle, grant foreground only. Signal: the guidance banner reads «grant background location», NOT «something went wrong» (F42), and if `startLocationUpdatesAsync` refuses, the toggle must fold back to OFF with no foreground-service notification — a toggle left ON with no stream is F36's ghost, and a nudge follows 60–75 s later. If the task does start (a mounted phone streams on foreground-only), the banner is the only signal to read here.
14. **Toggle OFF mid-ride (issue #141 — SUPERSEDED, do not run this step)**: #141 landed the fix — run its run sheet (`.claude/plans/driver-toggle-off-mid-ride-held.md` §Level 4 §1) instead; the toggle now springs back to ON with a banner and the pin keeps moving. *Historical, for the record only*: as written here the step force-assigned the driver from `/dispatch`, then tapped the toggle OFF, and expected to FAIL — the app tore the stream down before the api refused with `driver_on_ride`, so the pin froze on the board while the row still said `on_ride`, and one tap back ON recovered.
