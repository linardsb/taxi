...
| E7 | **Every** socket connect, first included → `GET /rides/:rideId`, which both refetches the status and joins the ride room (corrected at review round 1 — C1) | `use-ride-status.test.tsx` + `ride-lifecycle.integration.spec.ts` ("joins a socket opened AFTER the booking") + **Level 4 step 7** |
