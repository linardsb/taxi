...
- [Expo Location — `getCurrentPositionAsync`, foreground permissions](https://docs.expo.dev/versions/latest/sdk/location/)
  ...
  - Why: the rider app must request **only** `ACCESS_COARSE_LOCATION`/`ACCESS_FINE_LOCATION`. `ACCESS_BACKGROUND_LOCATION` must never appear in this manifest — the architecture doc's whole reason for two apps.
...
6. `AccessibilityInfo.announceForAccessibility` fires on: quote arrived, quote failed, ride requested, status changed, no-drivers timeout.
...
- **IMPLEMENT**: The `rider.*` block, LV first (it is the reference dictionary and `MessageKey` derives from it), then RU and EN with **exactly** the same keys.
  ...
  - `rider.a11y.{quote_arrived,quote_failed,ride_requested,status_changed}` — the announce strings.
...
- **IMPLEMENT**: `android.permissions: ["ACCESS_COARSE_LOCATION", "ACCESS_FINE_LOCATION"]` and nothing else. `ios.infoPlist.NSLocationWhenInUseUsageDescription` only.
...
- **IMPLEMENT**: Composes pickup row, dropoff row, saved places, `QuoteCard`, `PaymentChips`, and the Book button. Pickup is populated on mount from `expo-location`'s foreground `getCurrentPositionAsync` → `POST /geo/places/…`? **No** — reverse-geocoding is not on the widened routes. Use `expo-location`'s own `reverseGeocodeAsync` for the display string and the raw coordinates for the point. Permission denied, timeout, or an error all fall through to an empty pickup row reading `rider.book.where_to` — never a blocking error (D7).
...
- **IMPLEMENT — the reconnect fix (D4)**: on every `connect` event **after the first**, call `GET /rides/:rideId` and take the status from the response. The socket that just reconnected is **not** in the ride room (`roomsOnConnect` returns no ride room, and `joinRideRoom` only moved the sockets alive at creation), so without this the rider is deaf from the first blip onward.
...
| E7 | Socket reconnects → status refetched via `GET /rides/:rideId` | `use-ride-status.test.tsx` + **Level 4 step 7** |
