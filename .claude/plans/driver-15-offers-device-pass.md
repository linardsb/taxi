...
- `docs/runbooks/driver-device-day.md` (**whole file**, 667 lines at `d6207be`; 831 at this PR's head, which this PR's own +164 explains) — Why: the only copy of the run sheet. §Setup (78), §Steps (233), §Emulator route (531), §"Setup, once" (554), §"Injecting a position" (598), §"Running the steps on an emulator" (683), §"Also on this day" (352). **T2 edits line 362; T9 appends a §Result.** ⚠️ **Every absolute line number above is a snapshot, and this ticket's own §Result insertion moves the ones below it.** Re-derive any of them by grepping the quoted phrase or heading at head — never by adding an offset, since several were already low against `origin/main`. PR #265's F1 broke twelve of them exactly this way.
