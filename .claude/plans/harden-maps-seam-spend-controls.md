...
- **`TRACKING_VIEW_MAX_PER_WINDOW = 120` per 60 s.** One viewer polling every 5 s is 12/min, so this is 10 concurrent viewers on one token. #17's share-trip reuses the token, so the budget is genuinely shared across viewers — a family of three is 36/min, well inside it. If share-trip later fans out wider than ~10 simultaneous viewers, this is the number that breaks first, and it breaks *visibly* (a 429 on the page), not silently.
