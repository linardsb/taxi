...
- **IMPLEMENT**:
  ...
  5. Output (lines 129-130): "Report the PR number + URL, base ← head, and: **'Draft. CI flips it ready in about six minutes when green (`observed` 313–361 s over the last 8 `ci.yml` runs on 2026-09-10); then run `piv-review-pr <number>`, then a human approves.'**" (amended 2026-09-10: the figure shipped is 195–206 s / about 3.5 min — 313–361 was run wall; see AMENDMENTS)
...
- **IMPLEMENT**:
  ```yaml
    ...
    # 2026-09-10: 2 critical, 46 high, 17 moderate, 1 low — the `.advisories` map;
    # 2/47/18/1 is `metadata.vulnerabilities` and sums to 68, see AMENDMENTS);
    # a PR may not grow it.
...
1. **O1 flip.** From the feature branch run `piv-create-pr`. `gh pr view --json isDraft --jq .isDraft` → `true`. `gh run watch` the CI run (`expected` about 6 min: `check` `observed` 313–361 s over the last 8 runs; `ready` starts after it — amended 2026-09-10: about 3.5 min, `check` 195–206 s; see AMENDMENTS). Then `isDraft` → `false`. Record the run URL.
...
- 2026-09-10 (T7, after PR A #167 and PR #178 merged) — **T7 done; #165 has nothing owed.** Throwaway PR #179, `probe/audit-diff-165` off `e75c677`, three pushes, closed with `--delete-branch` after. `observed`, job `completed_at − started_at` from `gh run view --json jobs`: `lodash@4.17.20` at the root → run 34514881914, `audit-diff` failure in 10 s (`66 at base, 71 at HEAD`, five ids), `ready` failure, PR a draft; pin removed and `zod@3.25.76` declared at the root → run 34515354578, success in 10 s (`66 at base, 66 at HEAD, none new`), PR flipped ready; `package.json` and `pnpm-lock.yaml` restored to `main` → run 34515835893, success in 7 s on the short-circuit; PR A's own first run 34471269249 is the plan's "run URL 3", 7 s. Table with heads and job URLs on PR #167's body; runbook §2 row updated. Two deviations from T7's text: its second push, "revert the pin", restores the lockfile and takes the short-circuit, so the AC's expected case (green *despite the backlog*, both audits run) would never have executed — the `zod` declaration changes the lockfile without adding a package; and `lodash@4.17.21`, tried first for that case, carries 3 advisory ids of its own (`observed` locally, `66 at base, 69 at HEAD`), so it was dropped before pushing. Also seen: `pnpm install --lockfile-only` at the repo's pnpm 10.33.2 re-keys the `next`/`styled-jsx` and `eslint-plugin-import` peer suffixes (34 lines out, 20 in, no package added or removed), so `main`'s lockfile is not what a fresh resolution writes; `--frozen-lockfile` accepts it (every `check` job on #179 green).
...
- 2026-09-10 (PR #167 review, round-1 fixes) — **four figures and three guarantees in this plan are retired.** The plan's own copies are left as written; this entry is the correction, and the line numbers are where the stale text sits.
  ...
  - **The audit backlog breakdown is 2 critical / 46 high / 17 moderate / 1 low** and sums to the 66 ids the gate diffs (line 316, and the ci.yml comment it prescribes). The plan's NOTES table attributed 2/47/18/1 to `metadata.vulnerabilities` correctly; every downstream copy dropped the qualifier, and 2/47/18/1 sums to 68 (F9).
  - **"ready always means this head passed" is false** and is now written as *"the head CI last finished testing passed"*. Three routes broke the absolute: the window between a push and its run reaching `ready`; a head that never gets a run (#52); and a `gh run rerun` of an older run, which replays the original payload, cancels the live run through the shared concurrency group and would flip the PR at an untested head. The last is fixed in `ci.yml` (the `ready` job compares `github.event.pull_request.head.sha` against `gh pr view --json headRefOid` and refuses on mismatch); the first two are closed by branch protection, below (F3).
  - **T4's re-draft-on-every-push idea is deliberately NOT taken.** The review proposed a no-`needs` job running `gh pr ready --undo` at run start to close the window. It would silently undo the runbook's documented human bypass — a person clicks **Ready for review**, pushes a fix, and CI re-drafts before any gate result exists. Rejected on that ground; required status checks close the window without fighting the bypass.
  - **"No model has a path to the flip" / "those two routes are the whole surface" / "No session can do that" are not guarantees.** The hook matches text, so writing the flip into a script and running it, or editing the hook, `ci.yml` or the gate scripts, matches nothing. Guards 4 and 5 now also read write-tool text for non-`.md` files, match case-insensitively, cover bare `lgtm` and `convertPullRequestToDraft`, anchor the dismissal pattern on the PATCH rather than word order, and refuse `gh pr create` without `--draft`; the hook additionally fences writes to `.github/workflows/`, `.github/scripts/`, `.claude/hooks/` and `.claude/settings.json`. A hook guarding its own file is a mitigation, not a boundary — the runbook now says so (F4, F5, F8).
  - **Branch protection was refused for being PRIVATE, not for the plan** (lines 24, 38). The 403 was real on GitHub Free while private; the repo went public the same day and the APIs opened. **Applied 2026-09-10** (`observed`, `PUT …/branches/main/protection` then read back): required checks `check`, `audit-diff`, `codeql`, `strict: true`, `enforce_admins: false`, force-pushes and deletions off. That is the server-side control the plan wanted and could not have; runbook §5 (F1).
  - **`codeql-gate.sh`'s "a PR ref with no analysis is red" needed a code change to stay true.** Once the repo had any upload, the alerts endpoint stopped answering 404 "no analysis found" and began answering `[]` for a never-analysed ref, identical to a clean one — so the exit-1 branch was dead and "unscanned" would have read as "clean". The script now counts `code-scanning/analyses?ref=` first (F6). Its counts line is also validated before use: an unparseable one used to exit **0** (F7).

- 2026-09-10 (PR #167's first run) — **T4's token assumption was wrong.** `github.token` with `pull-requests: write` cannot run the draft mutations: `observed` run 34471269249, `ready` red with `GraphQL: Resource not accessible by integration (markPullRequestReadyForReview)` while the three gate jobs were green. The job now reads `secrets.PR_READY_TOKEN`, a **classic** `repo`-scope PAT of Linards' (runbook §1.1); no secret means `ready` red and the PR a draft. A fine-grained token with this repo and Pull requests: Read and write was refused twice on run 34472223208 (`Resource not accessible by personal access token`); the classic token flipped #167 on the third re-run, 12:21:42, as linardsb, and started no run. The `permissions: pull-requests: write` block is gone with it. The same run gave the first live figures: `check` 198 s, `audit-diff` 7 s (short-circuit), `codeql` 82 s with zero open alerts on the PR ref.
