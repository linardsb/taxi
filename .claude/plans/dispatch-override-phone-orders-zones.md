...
**Q2 — `accepted|arriving → requested` is a cross-surface contract change.**
Task A5 modifies `ALLOWED_TRANSITIONS`, which every surface reads. The rider and driver apps have not shipped, so the blast radius today is the tracking page and the api's own guards — but #15/#16/#17 inherit it. **Assumption:** a rider seeing their accepted ride return to "searching for a driver" is acceptable and honest. If it isn't, the alternative is reassign = cancel + rebook, which loses the ride id, the tracking token and the rider's SMS thread.
...
**Q4 — Dina's S9-1 console drawing is still outstanding.**
#18's own ticket says: "When Dina's S9-1 console drawing arrives, review this + #19 plans against it before executing." It has not arrived (precizējumi round, sent 2026-07-09, still empty per PRD §9). **Pre-execution gate:** check whether it has landed; if it has, review the Phase C zone grid and the Phase B form layout against it first. If it hasn't, proceed — the evidence report's Autocab/TaxiCaller anatomy is the substitute reference, and it is a strong one.
