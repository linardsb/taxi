...
**Source of truth: `RT` + payload schemas in `packages/shared/src/realtime-events.ts`.** Socket.IO, Redis adapter. Event names follow `domain:action`. Payloads are zod schemas; wire timestamps are ISO strings (`z.string().datetime()`), not `Date`.
...
| `ride:offer` | api → driver app | `RideOfferEvent` | Is `rideOfferSchema` itself: carries `quote` **and** `split`, so the driver sees the full fare and the commission line. Has `expiresAt`; decline/timeout triggers re-offer |
...
- Socket.IO generics come from `ClientToServerEvents` / `ServerToClientEvents` in the same file.
