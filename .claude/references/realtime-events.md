...
|---|---|---|---|
| `driver:location` | driver app → api (ping); api → dispatch board + rider during active ride (event) | `DriverLocationPing` (in) / `DriverLocationEvent` (out) | Two schemas — see below. Throttled by movement; live positions in Redis GEO, not Postgres. Inbound handler: `features/drivers/location/driver-location.gateway.ts` — identity from the JWT, and **the server stamps `at`, ignoring the client's**. A position is only recorded for a driver in the Redis online set, and is dropped from `findNearby` once older than `DRIVER_LOCATION_TTL_SECONDS` (a read-time filter, not a key TTL). Fan-out is the dispatch room only; the rider leg arrives with #11 |
| `driver:queue` | api → driver app | `DriverQueueEvent` | Live place in a geozone queue; `position` is 1-based |
| `ride:status` | api → ride room (rider + driver + dispatch) | `RideStatusEvent` | Emitted on every state-machine transition — which is why there is no separate `ride:requested`. First emitted by `features/rides` on creation (`previousStatus: null`); #11 emits it on every subsequent transition |
