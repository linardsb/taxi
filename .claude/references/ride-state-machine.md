...
- **Payment lock (Atis's hard rule)**: `isPaymentMethodLocked()` is true from `accepted` onward — the rider cannot change payment method once a driver has the job.
- **settled** means money movement finished (ledger entries written) — `completed` is the physical end of the ride, `settled` the financial one.
